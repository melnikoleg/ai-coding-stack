...
if git cat-file -e "${marker_commit}^{commit}" 2>/dev/null; then
  ...
  reason="ARCHITECTURE.md is out of date: source files changed since its base commit ${marker_commit}. Update it now, incrementally: (1) call the codebase-memory MCP tool index_repository for this project, then detect_changes with base=${marker_commit}; (2) update only the affected sections of ARCHITECTURE.md following the procedure in .claude/commands/architecture.md (regenerate the mermaid diagram only if module imports changed); (3) replace the first line of ARCHITECTURE.md with the fresh marker printed by running: bash .claude/hooks/arch-doc-state.sh (no arguments). Do not modify any other files. If detect_changes fails or the codebase-memory tools are unavailable, do a full regeneration per the same command file instead."
else
